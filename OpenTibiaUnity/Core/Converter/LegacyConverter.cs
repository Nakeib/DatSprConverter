...
namespace OpenTibiaUnity.Core.Converter
{
    ...
    public class LegacyConverter : IConverter
    {
        ...
        public Appearances m_appearances;
        public Assets.ContentData m_contentData;
        public Assets.ContentSprites m_contentSprites;
        ...
        public bool ReadData(int version, out Appearances appearances, out Assets.ContentData contentData)
        {
            string datFile = Path.Combine(version.ToString(), "Tibia.dat");
            if (!File.Exists(datFile))
            {
                Console.WriteLine("Tibia.dat doesn't exist");
                appearances = null;
                contentData = null;
                return false;
            }
            ...
            Console.Write("Processing Appearances...");
            var rawContentDat = File.ReadAllBytes(Path.Combine(version.ToString(), "Tibia.dat"));
            contentData = new Assets.ContentData(rawContentDat, version);
        ...
        public async Task<bool> ReadSpritesAsync(Appearances appearances, Assets.ContentSprites contentSprites)
        {
            int start = 0;
            SaveSprites(appearances.Outfits, ref start, contentSprites);
            SaveSprites(appearances.Effects, ref start, contentSprites);
        ...
        public bool ReadSprites(int version, Appearances appearances, out Assets.ContentSprites contentSprites)
        {
            string sprFile = Path.Combine(version.ToString(), "Tibia.spr");
            if (!File.Exists(sprFile))
            {
                Console.WriteLine("Tibia.spr doesn't exist");
                contentSprites = null;
                return false;
            }
            ...
            string resultPath = Path.Combine(version.ToString(), "result");

            Directory.CreateDirectory(Path.Combine(resultPath, "sprites"));

            var task = ReadSpritesAsync(appearances, contentSprites);
            task.Wait();
            Directory.Delete(Path.Combine(version.ToString(), "result"), true);
        ...
        public bool BeginMapping()
        {
            m_appearances = null;
            m_contentData = null;
            m_contentSprites = null;
            ReadData(m_ClientVersion, out m_appearances, out m_contentData);
            ReadSprites(m_ClientVersion, m_appearances, out m_contentSprites);
