...
namespace OpenTibiaUnity.Core.Assets
{
    public class ContentSprites
    {
        ...
        public Dictionary<uint, string> spritesMap;

        public ContentSprites(byte[] buffer, int clientVersion, bool alpha = false) {
            spritesMap = new Dictionary<uint, string>();
        ...
        private Bitmap RawGetSprite(uint id) {
            ...
            var buffer = (byte[])converter.ConvertTo(bitmap, typeof(byte[]));

            var md5 = MD5.Create();
            var hashMD5 = BitConverter.ToString(md5.ComputeHash(buffer));
            if (!spritesMap.ContainsKey(id))
                spritesMap.Add(id, hashMD5);
